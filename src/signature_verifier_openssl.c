...
)
{
  ...
  if (x == NULL) { cryptolens_set_error(e, 5, X, 0); goto error; }

  x->rsa = RSA_new();
  if (x->rsa == NULL) { cryptolens_set_error(e, 5, X, 0); goto error; }
#if OPENSSL_VERSION_NUMBER < 0x10100000L
  x->rsa->n = BN_new();
  x->rsa->e = BN_new();
...
)
{
  ...
  if (n == NULL) { cryptolens_set_error(e, 5, X, 0); goto error; }
#else
  ...
  if (exp_current == NULL) {
    ...
    exp = BN_new();
    if (exp == NULL) { BN_free(n); cryptolens_set_error(e, 5, X, 0); goto error; }
  } else {
  ...
  if (result != 1) { BN_free(n); BN_free(exp); cryptolens_set_error(e, 5, X, 0); goto error; }
#endif
...
)
{
  ...
  if (exp == NULL) { cryptolens_set_error(e, 5, X, 0); goto error; }
#else
  ...
  if (n_current == NULL) {
    ...
    n = BN_new();
    if (n == NULL) { BN_free(exp); cryptolens_set_error(e, 5, X, 0); goto error; }
  } else {
...
)
{
  ...
  if (o->rsa == NULL) { cryptolens_set_error(e, 5, X, 0); goto error; }
  ...
  ctx = EVP_MD_CTX_new();
#endif
  if (ctx == NULL) { cryptolens_set_error(e, 5, X, 0); goto error; }

  pkey = EVP_PKEY_new();
  if (pkey == NULL) { cryptolens_set_error(e, 5, X, 0); goto error; }

  r = EVP_PKEY_set1_RSA(pkey, o->rsa);
  if (r != 1) { cryptolens_set_error(e, 5, X, 0); goto error; }
